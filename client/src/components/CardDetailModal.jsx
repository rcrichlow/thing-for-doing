...
export default function CardDetailModal({ card, onClose, onUpdate, onDelete, isDeleting = false }) {
    ...
    return (
        ...
        >
            {saveError && (
                <div className="mb-4 rounded-md bg-red-900/20 p-3 text-sm text-red-300" role="alert">
                    {saveError}
                </div>
            )}
            <div className="mb-4">
                ...
                {titleEditing ? (
                    ...
                    <div
                        ...
                        className="cursor-pointer rounded-md px-3 py-2 text-sm hover:bg-zinc-800 text-zinc-100"
                        data-testid="card-detail-title-text"
                    >
                        {card.title || <span className="text-zinc-500 italic">Click to add title</span>}
                    </div>
                )}
            </div>
            ...
            <div>
                ...
                {descriptionEditing ? (
                    <textarea
                        ...
                        autoFocus
                        rows={5}
                        className="w-full rounded-md border border-zinc-700 bg-zinc-800 text-zinc-100 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 resize-none"
                    ...
                    <div
                        ...
                        className="cursor-pointer rounded-md px-3 py-2 text-sm hover:bg-zinc-800 min-h-[2rem] text-zinc-300"
                        data-testid="card-detail-description-text"
                    >
                        {card.description || <span className="text-zinc-500 italic">Click to add description</span>}
                    </div>
                )}
            </div>
        </Modal>
